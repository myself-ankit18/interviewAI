...
async function generateInterviewReportController(req, res) {
  try {
    ...
    if (!Array.isArray(aiResponse.technicalQuestions))
      aiResponse.technicalQuestions = [];
    if (!Array.isArray(aiResponse.behavioralQuestions))
...
async function generateResumePDFController(req, res) {
  try {
    ...
    const existingResume = interviewReport.enhancedResumes?.find(
      (r) => r.aiModel === aiModel,
    );
    if (existingResume) {
      return res.status(400).json({
    ...
    if (!result || !result.html) {
      ...
      return res.status(500).json({
        success: false,
        message:
          "AI agent failed to synthesize your resume. Please try a different model or try again later.",
      });
    }
    ...
    return res.status(500).json({
      success: false,
      message:
        error.message || "Failed to generate resume PDF. Please try again.",
    });
  }
}
...
async function downloadExistingResumePDFController(req, res) {
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="enhanced_resume_${modelName.replace(/\//g, "_")}.pdf"`,
  );
  return res.status(200).send(pdf);
}
...
async function generateProjectIdeasController(req, res) {
  ...
  const skillGaps = interviewReport.skillGaps.filter(
